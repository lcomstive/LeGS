...
namespace LEGS.Characters
{
	...
	public class Character : Entity, ICharacter, IAttributeHolder
	{
		...
		protected override void Awake()
		{
			...
			/// Flag all attributes as dirty to set <see cref="Attribute.CurrentValue"/>
			foreach(Attribute attribute in Attributes)
				attribute.SetDirty();

			Health = MaxHealth;

			m_StatusEffectChangeEventID = EventManager.Register<StatusEffectChangeArgs>(StatusEffectChangeArgs.EventName);
			m_EntityHealthChangeEventID = EventManager.Register<EntityHealthChangeEventArgs>(EntityHealthChangeEventArgs.EventName);
		}
		...
		private class CharacterEditor: Editor
		{
			private Character m_Target;

			private void OnEnable() => m_Target = (Character)target;
			...
			public override void OnInspectorGUI()
			{
				...
				GUILayout.Space(10);
				EditorGUILayout.HelpBox("When in play mode and editing attributes using the inspector, the internal values are not updated.\nUse this button to update all attributes on this character", MessageType.Info);
				if(GUILayout.Button(new GUIContent("Update Attributes")))
					foreach(Attribute attribute in m_Target.Attributes)
						attribute.SetDirty();
			}
		}
		#endif
